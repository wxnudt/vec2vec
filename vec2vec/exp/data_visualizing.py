...
def plot_embedding_2d(X, title=None):
    ...
    for i in range(X.shape[0]):
        ax.text(X[i, 0], X[i, 1],str(digits.target[i]),
                 color=plt.cm.Set1(y[i] / 8.), #color=plt.cm.Set1(y[i] / 10.),
...
# umap_ = umap.UMAP(n_neighbors, n_components=3, min_dist=0.3, metric='correlation')
umap_ = umap.UMAP(n_neighbors=100, n_components=3, min_dist=0.8, metric='correlation')
t0 = time()
X_umap = umap_.fit_transform(X)
print(X_umap.shape)
print(X_umap)
plot_embedding_2d(X_umap[:,0:2],"t-umap 2D")
plot_embedding_3d(X_umap,"t-umap 3D (time %.2fs)" %(time() - t0))
